...
exports.getParcoursById = asyncHandler(async (req, res, next) => {
  ...
  if (!parcours) {
    return next(
      new ErrorResponse(`Aucun parcours avec l'id suivant ${req.params.id}`),
      404
    );
  }
  res.status(200).json({
...
exports.createParcours = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp) {
    return next(
      new ErrorResponse(
        `Aucun bootcamp avec l'id suivant ${req.params.bootcampId}`
      ),
      404
    );
  }
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `Utilisateur ${req.user.id} non autorisé a ajouter un parcours a ce bootcamp ${bootcamp._id}`,
        401
      )
    );
  }
...
exports.updateParcours = asyncHandler(async (req, res, next) => {
  ...
  //Verifier que l utilisateur est bien le detenteur du bootcamp
  if (parcours.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `Utilisateur ${req.user.id} non autorisé a modifier ce parcours`,
        401
      )
    );
  }
...
exports.deleteParcours = asyncHandler(async (req, res, next) => {
  ...
  if (parcours.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `Utilisateur ${req.user.id} non autorisé a supprimer ce parcours`,
        401
      )
    );
  }
